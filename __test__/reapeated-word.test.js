...
let reapeatedWord = require('../challenges/repeatedWord/repeated-word');

describe('Reapeated word function test',()=>{
  it('the first text',()=>{
    let text = 'Once upon a time, there was a brave princess who...';
    expect(reapeatedWord(text)).toEqual('a');
  });
  it('the second text',()=>{
    ...
    expect(reapeatedWord(text)).toEqual('the');
  });
  it('the third text',()=>{
    let text = 'It was a queer, sultry summer, the summer they electrocuted the Rosenbergs, and I didn’t know what I was doing in New York..';
    expect(reapeatedWord(text)).toEqual('summer');
  });
});